...
const GET_DISTRIBUTION = gql`
  query SpeciesDistribution($canonicalName: String) {
    species(canonicalName: $canonicalName) {
      ...
      specimens(page: 1, pageSize: 1000, sorting: { sortable: STATUS, order: DESCENDING }, filters: []) {
        total
        records {
...
type Regions = {
  ibra: (Pick<RegionDistribution, "names"> & { dataset: Pick<DatasetDetails, "name"> })[];
  imcra: (Pick<RegionDistribution, "names"> & { dataset: Pick<DatasetDetails, "name"> })[];
};

type SpecimenMarker = Pick<
  SpecimenSummary,
  | "collectionRepositoryId"
  | "collectionRepositoryCode"
  | "institutionCode"
...
type GenomeMarker = Pick<WholeGenome, "recordId" | "latitude" | "longitude">;
type LociMarker = Pick<SpeciesMarker, "recordId" | "latitude" | "longitude">;
type ComponentMarker = Pick<GenomicComponent, "recordId" | "latitude" | "longitude">;
...
interface SpeciesQuery {
  species: {
    ...
    specimens: {
      total: number;
      records: SpecimenMarker[];
    };
    wholeGenomes: {
      total: number;
      records: GenomeMarker[];
    };
    markers: {
      total: number;
      records: LociMarker[];
    ...
    genomicComponents: {
      total: number;
      records: ComponentMarker[];
    };
  };
}

function fromSpecimenMarker(record: SpecimenMarker): Marker<null> {
  return {
    tooltip: [record.institutionCode, record.collectionRepositoryId ?? "unknown"].join(" "),
...
function fromGenomeMarker(record: GenomeMarker): Marker<null> {
  return {
    tooltip: record.recordId,
    latitude: record.latitude ?? 0,
    longitude: record.longitude ?? 0,
    color: [243, 117, 36, 220],
    type: Layer.WholeGenome,
  };
}

function fromLociMarker(record: LociMarker): Marker<null> {
  return {
    tooltip: record.recordId,
    latitude: record.latitude ?? 0,
    longitude: record.longitude ?? 0,
...
function fromComponentMarker(record: ComponentMarker): Marker<null> {
  return {
    tooltip: record.recordId,
    latitude: record.latitude ?? 0,
    longitude: record.longitude ?? 0,
    color: [185, 210, 145, 220],
    type: Layer.OtherData,
  };
}
...
export default function Page() {
  ...
  return <DistributionPage canonicalName={details.name} />;
}

function DistributionPage({ canonicalName }: { canonicalName: string }) {
  ...
  const { loading, error, data } = useQuery<SpeciesQuery>(GET_DISTRIBUTION, {
    variables: { canonicalName },
  });

  const specimens = data?.species.specimens.records.map(fromSpecimenMarker);
  const loci = data?.species.markers.records.map(fromLociMarker);
  const genome = data?.species.wholeGenomes.records.map(fromGenomeMarker);
  const components = data?.species.genomicComponents.records.map(fromComponentMarker);

  useEffect(() => {
    const combined = [
      ...(layers.specimens && specimens ? specimens : []),
      ...(layers.loci && loci ? loci : []),
      ...(layers.wholeGenome && genome ? genome : []),
      ...(layers.other && components ? components : []),
    ];
