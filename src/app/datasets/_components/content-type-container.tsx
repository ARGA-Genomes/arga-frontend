...
import {
  TableCardLayout,
  TableCardSwitch,
} from "@/components/table-card-switch";

// Local imports
import styles from "../../../components/record-list.module.css";
import classes from "./content-type-container.module.css";
...
const DatasetRow = memo(function DatasetRowComponent({ dataset, sourceLength, count }: DatasetRowProps) {
...
const CollectionCard = memo(function CollectionCardComponent({ collection }: { collection: Source }) {
  return (
    <Paper
      shadow="md"
      radius="lg"
...
const CollectionRow = memo(function CollectionRowComponent({ collection }: { collection: Source }) {
  return (
...
export function GroupedSources({ groups }: GroupedSourcesProps) {
  return (<Accordion
    variant="separated"
    radius="xl"
  ...
  return (
    ...
    >
      <Group justify='space-between' pr={'lg'}>
        <Accordion.Control pl="lg" py={4} onClick={() => setActive(!active)}>
